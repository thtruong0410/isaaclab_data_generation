...
"""
MimicGen-compatible environment config for the Franka open-door task.

Subtasks:
  1. grasp  - gripper engages the door knob/handle
  2. open   - pull the door open
"""
...
from .franka_open_door_env_cfg import FrankaOpenDoorEnvCfg, resolve_door_target_variants
...
def handle_is_grasped(
    ...
    if dist_threshold is None:
        dist_threshold = _get_env_float("OPEN_DOOR_MIMIC_GRASP_DIST_THRESHOLD", 0.10)
    if gripper_threshold is None:
        gripper_threshold = _get_env_float("OPEN_DOOR_MIMIC_GRIPPER_THRESHOLD", 0.055)
    if align_threshold is None:
        align_threshold = _get_env_float("OPEN_DOOR_MIMIC_ALIGN_THRESHOLD", 0.05)
    if require_wrap_alignment is None:
        require_wrap_alignment = os.getenv("OPEN_DOOR_MIMIC_REQUIRE_WRAP_ALIGNMENT", "0").strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
    ...
    handle_pos_all, handle_quat_all = _selected_handle_pose(env, cabinet_name=cabinet_name)
...
@configclass
class _OpenDoorSubtaskTermsCfg(ObsGroup):
...
@configclass
class FrankaOpenDoorMimicEnvCfg(FrankaOpenDoorEnvCfg, MimicEnvCfg):
    def __post_init__(self):
        super().__post_init__()
        ...
        self.datagen_config.name = "demo_src_open_door_isaac_lab"
        ...
        self.subtask_configs["franka"] = [
            SubTaskConfig(
                ...
                next_subtask_description="Open cabinet door",
            ),
            SubTaskConfig(
                object_ref="cabinet",
